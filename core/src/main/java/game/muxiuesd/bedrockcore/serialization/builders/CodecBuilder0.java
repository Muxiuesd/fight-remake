...
public class CodecBuilder0<T> {
    ...
    // 声明一个构造参数字段
    public <A> CodecBuilder1<T, A> paramField(String name, Function<T, A> getter, Codec<A> codec) {
        paramFields.add(new ParamField<>(name, getter, codec));
        return new CodecBuilder1<>(this);
    }

    // 普通字段（setter 注入），返回自身以便链式调用
    public <F> CodecBuilder0<T> field(String name, Function<T, F> getter, BiConsumer<T, F> setter, Codec<F> codec) {
