...
public class SimplexNoise2D {
    ...
    private static final int SEED = 42; // 固定种子，保证噪声一致性

    private static int[] generateGradient(int x, int y, int seed) {
    ...
    private static int[] getGradient(int x, int y) {
        String key = x + "," + y;
        if (gradientCache.containsKey(key)) {
            ...
            int[] gradient = generateGradient(x, y, SEED);
    ...
    private static void spawnNoiseMap(int startX, int startY) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                double value = getNorNoise(startX+x, startY+y, 0.08);
    ...
    public static double getNorNoise(double x, double y){
        double scl = 0.08;
        return getNorNoise(x, y, scl);
    }
    public static double getNorNoise(double x, double y, double scl){
        double nx = x * scl;
        double ny = y * scl;
