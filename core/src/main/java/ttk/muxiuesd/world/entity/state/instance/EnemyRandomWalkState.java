 ...
 * 敌人随机游走状态（通用）
 * */
public class EnemyRandomWalkState<T extends Enemy<?>> extends StateEnemy<T> {
    ...
    public void handle (World world, T entity, float delta) {
        updateAndCheckTimer(delta,
            () -> {
                //随机游走结束
                if (entity.checkTarget()) entity.setState(Enemy.STATE_ATTACK_TARGET);
                else entity.setState(Enemy.STATE_REST);
            },
            () -> {
                //还在游走状态;
     ...
     * 生成简单的随机游走路线（避开水面）
     * */
    public void randomWalkPath (World world, T entity) {
