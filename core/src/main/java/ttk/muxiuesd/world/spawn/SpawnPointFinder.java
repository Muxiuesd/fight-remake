 ...
 * 先用 {@link BiomeSampler} 的噪声场**零加载**预判候选区块群系是否可出生（canSpawn），
 * 只有可出生的陆地区块才用 {@link ChunkSystem#initChunk} 隔离生成确认其 canSpawn。
 * <p>
...
public class SpawnPointFinder {
    ...
    public Vector2 findLandSpawn () {
        ...
        // ② 螺旋扩散找第一个可出生的陆地区块（initChunk 隔离生成，不污染系统）
        Chunk spawnChunk = this.scanSpawnableChunk(baseX, baseY);
        if (spawnChunk == null) return new Vector2(0, 0);

        // ③ 在该区块内随机选一个非水方块格作为出生点
        return this.pickLandCell(spawnChunk);
    }

    /**
     * 以 (baseX, baseY) 为中心，螺旋向外扩散寻找可出生区块。
     * <p>
     * 优化：先噪声预判群系 canSpawn（零加载），只对可出生的陆地区块 initChunk 生成确认。
     * 生成公式确认后该区块将作为出生区块，由调用方（ChunkSystem）随后正式加载。
     */
    private Chunk scanSpawnableChunk (float baseX, float baseY) {
        ...
        for (int ring = 0; ring < MAX_RING; ring++) {
            ...
            for (ChunkPosition cp : ringCells) {
                // 零加载预判：该区块（用中心点）的群系是否可出生
                float centerWX = cp.getX() * Chunk.ChunkWidth + Chunk.ChunkWidth / 2f;
                float centerWY = cp.getY() * Chunk.ChunkHeight + Chunk.ChunkHeight / 2f;
                Biome biome = this.cs.getBiomeAt(centerWX, centerWY);
                if (biome == null || !biome.getCanSpawn()) continue;   //海洋/不可出生：跳过

                // 疑似可出生 → 隔离生成该区块确认 canSpawn
                //（initChunk 不 addChunk：不出现在系统中，不会被写盘保存）
                Chunk candidate = this.cs.initChunk(cp.getX(), cp.getY());
                if (candidate != null && candidate.getCanSpawn()) {
                    return candidate;
                }
            }
        }
    ...
    private Vector2 pickLandCell (Chunk chunk) {
        ...
        //区块内没有陆地格（理论上不会，canSpawn 群系必有陆地），退回区块中心
        return new Vector2(
