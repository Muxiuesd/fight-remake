...
public class AssetsLoader implements Disposable {
     ...
     * @param <T> 资源类型
     */
    public <T> void loadAsync(String fileName, Class<T> type, Runnable callback) {
            ...
            Gdx.app.postRunnable(() -> {
                try {
                    ...
                    if (assetManager.isLoaded(fileName, type)) {
                        ...
                        throw new IllegalStateException("资源加载失败: " + fileName);
                    }
                } catch (Exception e) {
    ...
    public <T> T get(String fileName, Class<T> type) {
        if (!assetManager.isLoaded(fileName, type)) {
            throw new IllegalStateException("资源未加载: " + fileName);
        }
