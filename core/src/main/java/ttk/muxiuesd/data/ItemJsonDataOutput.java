...
public class ItemJsonDataOutput extends JsonDataOutput {
    ...
    public void output (JsonDataWriter writer) {
        ...
        AbsFileUtil.createFile(Fight.PATH_SAVE, "item.json").writeString(json.prettyPrint(string), false);

        //读取测试
        String s = AbsFileUtil.readFileAsString(Fight.PATH_SAVE, "item.json");
        System.out.println(s);
        JsonDataReader dataReader = new JsonDataReader(s);
        String read = dataReader.readString(PropertyTypes.ITEM_USE_SOUND.getId());
